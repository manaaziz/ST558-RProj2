...
Then I looked at the numeric summaries of some of the quantitative variables as well as contingency tables of the qualitative variables.  
```{r, echo=FALSE}
summary(train[,c(1, 9, 15, 17, 18)])
```  
...
```  
After evaluating the predictions of the test data set from each model, the basic regression tree had an RMSE of `r tbl.rmse[1,1]` and the boosted tree model had an RMSE of `r tbl.rmse[2,1]`. I expected the boosted tree to have a lower RMSE, since we learned that ensemble trees tend to outperform single trees in terms of prediction.
